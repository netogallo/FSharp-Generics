...
previously. Befor doing so, however, we would like to give a brief
...
will be used for static checks wheras the notation $\tau_a :? \tau_b$
will be used for dynamic checks. \todo{Say something about when checks
  are dynamic vs static?}

As in most object oriented langagues, the .NET subtyping mechanism
...
One language feature particular to F\# is \emph{type
  providers}~\cite{typeProviders}. Typ providers in F\# define a
