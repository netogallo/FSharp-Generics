...
\end{lstlisting}
This code triggers the invokation of the type provider \verb+Provider+ which generates a type that is assigned the alias \verb+T+. The type \verb+T+ can now be used almost indistinguishly from any ordinary .Net type. The type provider api allows it to define constructors, static or regular fields and many other attributes that types contain. The use of this type is also typechecked by the compiler in the same way it happens with ordinary types.
\\\\
Type providers can accept optional and mandatory static parameters. These parameters are very different from generic types. They are values of types \verb+bool+, \verb+int+ or \verb+string+ and can only be literal values. This means they cannot accept types (as the case of generic types) as arguments.
