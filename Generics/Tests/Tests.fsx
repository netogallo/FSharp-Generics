#r @"bin\Debug\Generics.dll"
#r @"bin\Debug\Provider.dll"
...
type Everywhere<'t,'v>(f : 'v -> 'v) =
    ...
    member x.Everywhere(c : Id<'t>) =
        let g = Generic<'t>()
        Id<'t>(x.Everywhere(g.To c.Elem) :?> Meta |> g.From)
